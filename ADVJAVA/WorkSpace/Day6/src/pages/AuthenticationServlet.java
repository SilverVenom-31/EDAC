...
public class AuthenticationServlet extends HttpServlet {
	...
	public AuthenticationServlet() {
		System.out.println("In constructor " + getClass().getName() + " " + getServletConfig());// null
	}
	...
	public void init() throws ServletException {
		...
		try {
			// get servlet specific init param from ServletConfig
			ServletConfig cfg = getServletConfig();
			dao = new CustomerImpl(cfg.getInitParameter("drvr"), cfg.getInitParameter("url"),
					cfg.getInitParameter("user"), cfg.getInitParameter("password"));
		} catch (Exception e) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (cus == null) {
				...
				// use server pull: forward to navigate client to category page in SAME request,
				// managed by WC server space
